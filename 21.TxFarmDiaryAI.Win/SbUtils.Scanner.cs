...
namespace TxFarmDiaryAI.Win
{
    ...
    {
        public static List<ScannerDevice>? GetScannerDevices()
        {
            List<ScannerDevice>? Result = null;
            ...
            foreach (WIA.DeviceInfo info in deviceManager.DeviceInfos)
            {
                if (info != null && info.DeviceID.IsNullOrWhiteSpaceEx() != true && info.Properties != null && info.Properties.Count > 0)
                {
                    ...
                    Result.Add(new ScannerDevice { DeviceID = strDeviceID, DeviceName = strDeviceName });
                }
            }
        ...
        internal static bool ConnectToScanner(ScannerDevice selectedScanner)
        {
